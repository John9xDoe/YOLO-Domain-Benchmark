...
def generate_object(fig_type=None, h_bg=640, w_bg=640, color_fig=None, color_bg=None, base_size=100, save=True, save_path=None, filename='datasample', part='train'):
    ...
    if fig_type is None:
        fig_type = random.choice(list(classes.keys()))
    ...
    if save:
        if save_path is None:
            _save_data_sample(classes[fig_type], img, label, part=part, filename=filename)
        else:
            _save_data_sample(classes[fig_type], img, label, part=part, filename=filename,path=save_path)
...
def _save_data_sample(class_id, img, label, path='data', filename='datasample', meta=False, part='train'):
    cv2.imwrite(f'{path}/images/{part}/{filename}.png', img)

    with open(f'{path}/labels/{part}/{filename}.txt', 'w') as f:
        bbox = label['rel'][0]
        f.write(f"{class_id} {bbox[0]} {bbox[1]} {bbox[2]} {bbox[3]}")

    if meta:
        with open(f'{path}/meta/{part}/{filename}.json', 'w') as f:
